...
def infix_to_rpn(expression):
    ...
    associativity = {
        '~': 'R',  # унарный минус - правоассоциативный
    ...
    while i < n:
        ...
        if ch in '+-*/^()':
            if ch == '(':
                ...
                if ch == '-':
                    # Унарный минус, если:
                    # - токен в начале выражения
                    # - предыдущий токен - оператор или '('
                    # - предыдущий токен - функция
                    is_unary = (last_token_type is None or 
                               last_token_type in ['operator', 'lparen', 'function'])
                    if is_unary:
                        tokens.append('~')  # Используем ~ для унарного минуса
